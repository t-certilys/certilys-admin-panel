...
export default function OrderDetailPage() {
  ...
  async function handleConfirm() {
    ...
    try {
      await simulateApiCall(dialog.type, order.id, dialog.reason);
          ...
          updated.events = [
            {
              ...
              description: `L'accès à la formation a été révoqué manuellement par l'administrateur. Motif : ${dialog.reason}`,
            },
            ...prev.events,
          ];
          ...
          <div className="flex max-h-[inherit] flex-col">
            <DialogHeader className="shrink-0 px-6 pt-6 pb-4 sm:px-7 text-left">
              <DialogTitle className="flex items-center gap-2 text-base font-semibold">
                <HugeiconsIcon
                  icon={dialogCfg?.icon ?? InvoiceIcon}
                  className={`size-4 shrink-0 ${dialog.type === "revoke-access" ? "text-destructive" : "text-primary"}`}
                  size={16}
                  strokeWidth={1.5}
                />
                {dialogCfg?.label}
            ...
            <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-6 py-5 sm:px-7 sm:py-5 space-y-4">
              {/* Facturation Recap - Grid responsive à 3 colonnes */}
              <div className="grid gap-3 sm:grid-cols-[auto_1fr_auto] rounded-xl bg-muted/60 p-4 border border-border/40 min-w-0 overflow-hidden">
                ...
                <div className="min-w-0 space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground truncate" title={`${order.apprenant.name} · ${order.formation.title}`}>
                    {order.apprenant.name} · {order.formation.title}
                  </p>
                </div>
              ...
              {dialogCfg?.requiresReason && (
                <div className="space-y-2">
                  ...
                  />
                  {dialog.reason.trim().length > 0 && dialog.reason.trim().length < 10 && (
            ...
            <DialogFooter className="shrink-0 border-t bg-background px-6 py-6 sm:px-7 sm:py-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              <Button variant="outline" onClick={closeDialog} disabled={dialog.loading} className="w-full sm:w-auto">
                Annuler
              </Button>
              <Button
