...
export default function UserDetailPage() {
  ...
  async function handleConfirm() {
    ...
    try {
      await simulateApiCall(dialog.type, user.id, dialog.reason);
      ...
      setUser((prev) => {
        ...
        if (dialog.type === "suspend") {
          return {
            ...
            suspensionReason: dialog.reason,
  ...
  const canConfirm = !dialogCfg?.requiresReason || dialog.reason.trim().length >= 10;
          ...
          <DialogContent className="w-[min(calc(100vw-2rem),46rem)] max-h-[min(760px,calc(100dvh-2rem))] overflow-hidden rounded-2xl p-0 border border-border/60">
            <div className="flex max-h-[inherit] flex-col">
              ...
              <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-6 py-5 sm:px-7 sm:py-5 space-y-4">
                ...
                <div className="flex items-center gap-3 rounded-xl bg-muted/60 p-4 border border-border/40">
                  <div
                    className={`flex size-10 shrink-0 items-center justify-center rounded-xl text-sm font-semibold ${user.avatarColor}`}
                  >
                    {user.initials}
                  </div>
                  <div className="min-w-0">
                ...
                {dialogCfg?.requiresReason && (
                  <div className="space-y-2">
                    <Label htmlFor="action-reason-user" className="text-xs font-medium text-foreground">
                      {dialogCfg.reasonLabel}
                      <span className="ml-1 text-destructive">*</span>
                    </Label>
                    <Textarea
                      id="action-reason-user"
                      value={dialog.reason}
                      onChange={(e) =>
                    ...
                    {dialog.reason.trim().length > 0 &&
                      dialog.reason.trim().length < 10 && (
                        <p className="text-xs text-destructive">
                          Minimum 10 caractères requis (actuellement{" "}
                          {dialog.reason.trim().length}).
                        </p>
                      )}
                  </div>
                )}
              ...
              <DialogFooter className="shrink-0 border-t bg-background px-6 py-6 sm:px-7 sm:py-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                ...
                <Button
                  ...
                  disabled={dialog.loading || !canConfirm}
