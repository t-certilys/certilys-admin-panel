      ...
      {courseKpis.map((kpi) => {
        ...
        return (
          <button
            ...
            type="button"
            onClick={() => onStatusFilter(kpi.status)}
          ...
          <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-6 py-5 sm:px-7 sm:py-5 space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-[auto_1fr_auto] rounded-xl bg-muted/60 p-4 border border-border/40 min-w-0 overflow-hidden">
              ...
              <div className="min-w-0 space-y-1">
                <p className="text-sm font-semibold text-foreground truncate" title={state.course.title}>
                  {state.course.title}
                </p>
                <p className="text-xs text-muted-foreground truncate" title={`${state.course.instructorName} · ${state.course.category}`}>
                  {state.course.instructorName} · {state.course.category}
                </p>
              </div>
              <div className="shrink-0 self-start sm:self-center">
                <StatusBadge status={state.course.status} />
              </div>
            </div>
...
export default function CoursesPage() {
  ...
  async function handleConfirm() {
    ...
    try {
      await simulateApiCall(dialog.type, dialog.course.id, dialog.reason);
        ...
        prev.map((item) =>
          item.id === dialog.course!.id
            ? {
                ...item,
                status: newStatus,
                lastDecisionReason: dialog.reason || item.lastDecisionReason,
